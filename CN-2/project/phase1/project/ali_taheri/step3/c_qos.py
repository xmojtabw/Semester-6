...
class SimpleIPRouter(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        if datapath.id == 1:
            self.add_meter(datapath, meter_id=1, rate_kbps=1000)
    ...
    def handle_ip(self, pkt, datapath, in_port, dpid):
        ...
        if out_port:
            ...
            if dpid == 1 and (dst_ip == '10.0.0.1' or src_ip == '10.0.0.1'):
                inst = [
    ...
    def add_meter(self, datapath, meter_id, rate_kbps):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser

        bands = [parser.OFPMeterBandDrop(rate=rate_kbps, burst_size=rate_kbps // 10)]
        ...
        self.logger.info(f"{YELLOW}Meter {meter_id} added to limit rate to {rate_kbps} Kbps{RESET}")
        time.sleep(SLEEP_TIME)
