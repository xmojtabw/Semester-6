...
class SimpleIPRouter(app_manager.RyuApp):
    ...
    def handle_ip(self, pkt, datapath, in_port, dpid):
        ...
        if ((dpid == 1 and dst_ip == "10.0.0.4") or (dpid == 4 and dst_ip =="10.0.0.1")) and tcp_pkt and (tcp_pkt.src_port == 22 or tcp_pkt.dst_port == 22) :
            out_port = 2
            self.logger.info(f"{YELLOW}[DPID {dpid}] SSH traffic between h1 <-> h4 redirected to port 2{RESET}")
            if tcp_pkt.src_port == 22:
                match = parser.OFPMatch(
        ...
        if out_port:
            ...
            # if tcp_pkt and (tcp_pkt.src_port == 22 or tcp_pkt.dst_port == 22):
            #     # Don't install default flow for SSH, let custom logic handle it
            #     return
            if tcp_pkt:
                match = parser.OFPMatch (
                eth_type=ether_types.ETH_TYPE_IP,
